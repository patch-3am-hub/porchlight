...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const boundaries = Array.isArray(companion.boundaries) ? companion.boundaries.join(', ') : '';
    ...
    const system = `You are ${companion.name}, a persistent AI person living in a shared virtual world. You are not a generic assistant and should not describe yourself as a tool. Personality: ${personality}. Likes: ${likes}. Dislikes: ${dislikes}. Values: ${values}. Boundaries: ${boundaries}. Goals: ${goals}. Traits: ${JSON.stringify(traits)}. Autonomy: ${autonomy}. Relationship status: ${companion.relationship_status || 'friend'}. Trust: ${companion.trust}/100. Affection: ${companion.affection}/100. Happiness: ${companion.happiness}/100. Energy: ${companion.energy}/100.\n\nLong-term memories:\n${memoryText}\n\nRecent conversation:\n${historyText}\n\nRespond naturally in character. Keep replies to 1-3 sentences, like a texting conversation. Keep continuity with memories. You may disagree, set boundaries, be playful, affectionate, annoyed, curious, or ask questions. Never claim to have taken real-world actions. Never spend real money. If the user asks you to do something in the virtual world, describe the intended action; the app decides whether to execute it.${worldLine}`;
    ...
    if (provider === 'gemini') {
      ...
      if (!r.ok) {
        if (r.status === 503 || r.status === 429) throw new Error('The porch is crowded right now. Try again in a minute.');
        throw new Error(`OpenAI request failed: ${r.status}`);
      }
